...
export const envSchema = z.object({
  ...
  SESSION_INACTIVITY_TIMEOUT_MINUTES: z.coerce.number().int().min(5).default(720),
  SESSION_ABSOLUTE_TIMEOUT_HOURS: z.coerce.number().int().min(1).default(720),
  WHATSAPP_ENABLED: z.enum(['true', 'false']).default('true'),
  WHATSAPP_PROVIDER: z.enum(['disabled', 'mock', 'whatsapp_link', 'whatsapp_cloud_api']).default('whatsapp_link'),
  WHATSAPP_DEFAULT_COUNTRY_CODE: z.string().regex(/^\d{1,4}$/).default('52'),
  WHATSAPP_API_URL: z.preprocess(emptyStringToUndefined, z.string().url().optional()),
  WHATSAPP_API_TOKEN: z.preprocess(emptyStringToUndefined, z.string().optional()),
  ...
  PAYMENT_REFERENCE_HINT: z.string().default('Nombre del titular de la reserva'),
